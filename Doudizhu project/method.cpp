        ...
        if (beat)
        {
            ...
            for (CardPoint point = CardPoint(basePoint + 1); point <= Card_Q; point = CardPoint(point + 1))
            {
                ...
                for (int i = 0; i < extra; i++)
                {
                    ...
                    if (cards.isEmpty() || (point + i >= Card_2))	// the sequence is broken, or the cards reach at 2
                    {
                        seqPairFound = false;
                        seqPair.clear();
                        break;
...
QList<QList<card> > Method::FindCardsByCount(int count)
{
    ...
    if (count < 1 || count > 4)     return cardsList;   //count is not valid
...
{
    Hand hand(m_cards);
    if (hand.getHandType() != Hand_Unknown)		// when only one card left,play it
    {
        return m_cards;
    }
    ...
    if (hasTriple)
    {
        if (Hand(tripleArray[0]).getBasePoint() < Card_A)
        {
            ...
            return tripleArray[0];		// no other better cards to play, then play Triplet
        }
    }

    if (hasSeqPair)		// play a longest Sequence
    {
...
{
    // firstly prepare Straight, and try to play other cards
    ...
    if (hand.getHandType() == Hand_Single)	// If play Single, try to play the card that the count is one
    {
    ...
    else if (hand.getHandType() == Hand_Pair)	// If play Pair, try to pley card that the count is two
    {
    ...
    else	// if there is not better strategy, then select cards from the Straight
    {
    ...
    // When the opponent has less than 3 cards, if Bomb is possible then play Bomb
...
bool Method::WhetherToBeat(QList<card> &myCards)
{
    ...
    if (m_player->getRole() == hitPlayer->getRole())		// the target of attacking is a teammate
    {
        QList<card> left = m_cards;
    ...
    else	// the target of attacking is not a teammate
    {
        Hand myHand(myCards);

        if ( (myHand.getHandType() == Hand_Triple_Single || myHand.getHandType() == Hand_Triple_Pair) &&
             (myHand.getBasePoint() == Card_2) )	// do not play three 2s
        {
            return false;
        }
